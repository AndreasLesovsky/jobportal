...
export class HomeThreeComponent implements AfterViewInit, OnDestroy {
  ...
  private createLights(mode: string): void {
    ...
    if (mode === 'dark') {
      this.dirLight = new DirectionalLight(0x8899cc, 7.5);
      this.dirLight.position.set(280, 150, -30);
      this.renderer.toneMappingExposure = 0.1;
    } else {
      this.dirLight = new DirectionalLight(0xffe0b0, 10);
  ...
  private updateMonitorMaterials(mode: string): void {
    ...
    this.model.traverse((obj) => {
      if (obj instanceof Mesh && obj.name.includes('Computers_Monitor')) {
  ...
  private setupScrollAnimation(): void {
    ...
    const steps = isMobile
      ? [ // Mobile
        new Vector3(265, 22, 23),
        new Vector3(255, 21, -5),
        new Vector3(228, 21, -5),
        new Vector3(186, 22, -96),
      ]
      : [ // Desktop
        new Vector3(265, 22, 23),
        new Vector3(246, 20, -12),
        new Vector3(222, 20, -16),
        new Vector3(186, 22, -96),
      ];
    ...
    const lookTargets = isMobile
      ? [ // Mobile
        new Vector3(-1200, 0, -800),
        new Vector3(250, 5, -250),
        new Vector3(250, 5, -250),
        new Vector3(400, -30, -120),
      ]
      : [ // Desktop
        new Vector3(-1200, 0, -800),
        new Vector3(410, 10, -600),
        new Vector3(425, 5, -420),
        new Vector3(400, -30, -120),
      ];
    ...
    // Catmull-Rom-Kurven 
    const positionCurve = new CatmullRomCurve3(steps, false, 'catmullrom', 0.1);
    const lookCurve = new CatmullRomCurve3(lookTargets, false, 'catmullrom', 0.2);

    // Für FOV Vektor3 nehmen, nur X nutzen
    const fovCurve = new CatmullRomCurve3(
      fovs.map(f => new Vector3(f, 0, 0)),
      false,
      'catmullrom',
      0.45
    );
